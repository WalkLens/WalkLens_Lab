...
// com.microsoft.mixedreality.openxr package is included in the Unity project.  ASA_UNITY_OPENXR_OPT_OUT is intended
// as a safety valve for projects which have the OpenXR plugin but are using the WindowsMR plugin for whatever reason
#if (UNITY_WSA || WINDOWS_UWP) && ASA_UNITY_USE_OPENXR && !ASA_UNITY_OPENXR_OPT_OUT
#define USING_OPENXR
#endif
#endif

#if UNITY_IOS || UNITY_ANDROID || USING_OPENXR
#define USING_ARFOUNDATION_EVENT
#endif

#if UNITY_IOS || UNITY_ANDROID
#define USING_CAMERA_FRAMES
#endif
...
#if USING_ARFOUNDATION_EVENT
using System.Collections.Generic;
using UnityEngine.XR.ARSubsystems;
#endif

#if ARFOUNDATION_5_PRESENT
...
#endif // USING_ARFOUNDATION_EVENT
...
#pragma warning restore CS1998
        ...
        /// The event data.
        /// </param>
        protected virtual async void OnAnchorLocated(object sender, AnchorLocatedEventArgs args)
        {
...
#if USING_ARFOUNDATION_EVENT
#if USING_OPENXR
                // TrackableId is used for mapping here since OpenXrAnchor handle might have changed and 
                // the cloudSpatialAnchor identifier is now available.
                string invalidTrackableId = TrackableId.invalidId.ToString();
                string trackableId = invalidTrackableId;

                // Retry trackableId retrieval in the event that it is unavailable at the time of call
...
#else
                // On mobile platforms and Hololens OpenXR, ARAnchors are automatically surfaced via the ARAnchorManager
...
#endif // USING_OPENXR || !(USING_OPENXR)
                lock (pendingMobileEventArgs)
                {
                    pendingMobileEventArgs.Add(args);
                }
#elif WINDOWS_UWP || UNITY_WSA
                // On HoloLens WindowsMR, ARAnchors are not automatically surfaced through the ARAnchorManager
...
#endif // ENABLE_WINMD_SUPPORT || (!UNITY_EDITOR)
#endif // USING_ARFOUNDATION_EVENT || (WINDOWS_UWP || UNITY_WSA)
            }
            else
            {
...
#pragma warning restore CS1998
        ...
        public void StopSession()
        {
            ...
            // unsubscribe from session status events
            session.SessionUpdated -= OnSessionUpdated;
